    <?php
        $db = new mysqli($dbsrv,$dbuser,$passwd,$database);
            if($db->connect_errno)
                ...
                {
                 $get_minmaxavg = "SELECT date(datetime),Max(Temperatur),AVG(Temperatur),Min(Temperatur),Regen from wetterdaten01  PARTITION (p_wetterdaten_act_05) group by date(datetime)";
